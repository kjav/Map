...
function interpolate(array, width, height, scale) {
  ...
  for (var i = 0; i < (width - 4) * scale; i++) {
    ...
    for (var j = 0; j < (height - 4) * scale; j++) {
      ...
      y0 = cubic(array[i_x - 1][i_y - 1], array[i_x - 0][i_y - 1],
                       array[i_x + 1][i_y - 1], array[i_x + 2][i_y - 1])(x - i_x);

      y1 = cubic(array[i_x - 1][i_y - 0], array[i_x - 0][i_y - 0],
                       array[i_x + 1][i_y - 0], array[i_x + 2][i_y - 0])(x - i_x);

      y2 = cubic(array[i_x - 1][i_y + 1], array[i_x - 0][i_y + 1],
                       array[i_x + 1][i_y + 1], array[i_x + 2][i_y + 1])(x - i_x);

      y3 = cubic(array[i_x - 1][i_y + 2], array[i_x - 0][i_y + 2],
                       array[i_x + 1][i_y + 2], array[i_x + 2][i_y + 2])(x - i_x);

      var f = cubic(y0,y1,y2,y3);
